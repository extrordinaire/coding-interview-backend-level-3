...
import { z, minLength } from '@zod/mini';
import pgClient from 'postgres';
import { eq as eq$1 } from 'drizzle-orm';
...
async function async_unwrap(promise) {
  try {
    ...
    const error = maybe_error instanceof Error ? maybe_error : new Error(String(maybe_error));
    return {
      value: null,
      error,
...
const define_routes = (server) => {
  ...
  server.route({
    ...
    handler: async (request, h) => {
      ...
      const { error, value } = await async_unwrap(
        db.select().from(items).where(eq$1(items.id, Number(id))).limit(1).then((rows) => rows[0])
      );
      if (error) {
        return internal(error);
      }
  ...
  server.route({
    ...
    handler: async (request, h) => {
      const { value, error } = await async_unwrap(
        db.insert(items).values(request.payload).returning()
      );
      if (error) {
  ...
  server.route({
    ...
    handler: async (request, h) => {
      const { id } = request.params;
      const { value, error } = await async_unwrap(
        db.update(items).set(request.payload).where(eq$1(items.id, Number(id))).returning()
      );
      if (error) {
        internal(error);
      }
  ...
  server.route({
    ...
    handler: async (request, h) => {
      const { id } = request.params;
      const { value, error } = await async_unwrap(
        db.delete(items).where(eq$1(items.id, Number(id))).returning()
      );
      if (error) {
        return internal(error);
      }
...
const zod_adapter = {
  compile: (schema) => {
    return zod_validator(schema);
  }
};
const zod_validator = (schema, boom) => async (value, _) => {
  ...
  if (parse_error) {
    ...
    throw new Error(void 0, { cause: formatted_zod_error });
  }
  return parsed_data;
};
